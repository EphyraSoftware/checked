...
import {Create, EntryHash, HoloHashed, Record} from "@holochain/client";
...
import {
    ...
    testAppPath,
    sampleMiniSignProof,
    sampleMiniSignProofSignature, sampleMiniSignProofSignature2, sampleMiniSignProof2, sampleMiniSignKey2,
} from "./common.js";
...
test("Mark a key as compromised", async () => {
    await runScenario(async (scenario) => {
        ...
        assert.equal(mark["Compromised"].note, "I think someone is using my private key!");
...
test("Mark a key as rotated", async () => {
    await runScenario(async (scenario) => {
        ...
        assert.deepEqual(mark["Rotated"].new_verification_key_dist_address, new_vf_key_dist_address);

        assert.equal(responses[1].verification_key_dist.marks.length, 0);
        assert.equal(responses[1].verification_key_dist.verification_key, sampleMiniSignKey2().trim());
