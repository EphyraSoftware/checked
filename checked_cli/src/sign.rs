...
pub fn sign(sign_args: SignArgs) -> anyhow::Result<()> {
    ...
    if !sign_args.file.metadata()?.is_file() {
        anyhow::bail!("Value provided for `--file` is not a file - {:?}", sign_args.file);
    }

    let output_path = sign_args.output.clone().unwrap_or_else(|| {
        let p = sign_args.file.clone();
        p.with_extension(format!("{}.minisig", sign_args.file.extension().unwrap_or_default().to_str().unwrap_or_default()))
    });
    ...
    let pk = match PublicKey::from_file(&vk_path) {
        Ok(pk) => Some(pk),
        Err(e) => {
    ...
    let sig = minisign::sign(
        pk.as_ref(),
        &sk,
        &mut std::fs::File::open(&sign_args.file)?,
        None,
        None,
    )?;
